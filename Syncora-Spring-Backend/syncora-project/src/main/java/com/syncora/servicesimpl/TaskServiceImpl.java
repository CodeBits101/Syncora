...
public class TaskServiceImpl implements TaskService {
	...
	public ApiResponse createTask(TaskReqDto dto, Long id) {
		...
		if (assignedToEmp.getEmpRole() != EmployeeType.ROLE_DEVELOPER)
			throw new ApiException("The assigned user is not developer");
		else {
		...
		if (dto.getStoryId() != null) {
			...
			if (story.getCurrentSprint() != null) {
				Sprint sprint = sprintRepo.findById(story.getCurrentSprint().getId())
						.orElseThrow(() -> new ResourceNotFoundException("Sprint does not exist by this id"));
				createdTask.setSprint(sprint);
			}
		}
...
public List<BacklogItemDto> getBacklogTasks(Long projectId) {
	...
	public List<TaskRespDto> getTaskByStatus(Long id) {
		Story story = storyRepo.findById(id).orElseThrow(() -> new ResourceNotFoundException("Sprint Does not exist"));
		List<TaskRespDto> taskList = taskRepo.findByStory(story).stream().map(task -> {
