    ...
    <Card ref={setNodeRef} {...attributes} {...listeners} sx={style} data-card-id={id}>
      <CardContent sx={{ p: 1.5, height: '100%', position: 'relative', overflow: 'hidden' }}>
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
          <Typography variant="subtitle2" sx={{ fontWeight: 600, color: isDragging ? '#1E4DB7' : 'inherit' }}>
            {task.title}
          </Typography>
          <Box display="flex" alignItems="center" gap={1}>
            {typeIcons[task.type]}
            {task.type === 'subtask' && (
              <Typography variant="caption" sx={{ color: '#1E4DB7', fontWeight: 700, letterSpacing: 0.5 }}>
                Subtask
              </Typography>
            )}
            <Box width={10} height={10} borderRadius="50%"
              bgcolor={priorityColors[(task.priority || '').toLowerCase()] || priorityColors.default}
